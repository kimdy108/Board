...
const moveRegistPage = () => {
  router.push({ name: 'StackRegist' }).catch(() => {
    console.log('StackRegist Error')
  })
}

const moveInfoPage = (uuid: string) => {
  router.push({ name: 'StackInfo', state: {uuid: uuid} }).catch(() => {
...
const getStackList = async () => {
  ...
  const stackResult: responseData = await ApiService.requestAPI({
    headers: reqHeader,
    method: 'GET',
    url: `/board/post/list/page`,
    params: reqParams
  })
  if (stackResult.retStatus) {
    if (stackResult.retData.postContents && Array.isArray(stackResult.retData.postContents)) {
      for (let i = 0; i < stackResult.retData.postContents.length; i++) {
        if (stackResult.retData.postContents[i] && stackResult.retData.postContents[i].insertDate) {
          stackResult.retData.postContents[i].insertDate = stackResult.retData.postContents[i].insertDate.replace("T", " ")
        }
      }
    }

    contents.value = stackResult.retData.postContents || []
    totalCount.value = stackResult.retData.totalCount || 0
  }
}
</script>